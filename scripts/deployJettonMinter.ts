...
export async function run(provider: NetworkProvider) {
    ...
    const adminAddress =  Address.parseFriendly("0QAjwuTLdeRxuW6eoewhDViXunaWW0upaGKYHBwTP-mhYi1-"); //await promptUserFriendlyAddress("Enter the address of the jetton owner (admin):", ui, isTestnet);
    ...
    const jettonMetadataUri = "https://task-tracking.com/jetton-metadata.json";
    ...
    const minter = provider.open(JettonMinter.createFromConfig({
            ...
            wallet_code: jettonWalletCodeRaw,
            jetton_content: {uri: jettonMetadataUri}
        },
        await compile('JettonMinter')));

    await minter.sendDeploy(provider.sender(), toNano("1.5")); // send 1.5 TON even more
}
